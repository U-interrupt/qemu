...
{   
    qemu_log_mask(LOG_UNIMP, "write_ustatus %lx\n", val);
                                       ...
                                       target_ulong write_mask)
{
    ...
    /*
    if (csrno == 0x041) {
        qemu_log("PC=0x%lx ", env->pc);
        qemu_log("BADADDR=0x%lx ", env->badaddr);
        if (write_mask) {
            qemu_log("CSR write 0x%x\n", csrno);
        } else {
            qemu_log("CSR read 0x%x\n", csrno);
        }
    }
    */
...
riscv_csr_operations csr_ops[CSR_TABLE_SIZE] = {
    ...
    [CSR_SUICFG]   = { "suicfg",   nmode, read_suicfg,   write_suicfg   },
